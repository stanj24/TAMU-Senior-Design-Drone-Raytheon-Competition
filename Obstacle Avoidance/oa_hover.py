...
debug_enable = 1 # add an option if debug_enable = 1 to display the images
...
def check_obstacles(dist_array):
    ...
    for i in range(distances_array_length):
        obst_present == False
        # is obstacle present?
        if dist_array[i] < 100:  # distance to obstacle is less than 1 m away
            obst_present = True
            vehicle.mode = VehicleMode("LAND") # Land the drone immediately
        else:
            forward_GPS_point(2) # move forward 2m  ### ADD A CONTINGENCY SO IT WILL DEFINITELY LAND AFTER A CERTAIN POINT
...
while counter < 10:
    ...
    dist_array = distances_from_depth_image(obstacle_line_height, depth_mat, distances, DEPTH_RANGE_M[0], DEPTH_RANGE_M[1],
                                     obstacle_line_thickness_pixel)
